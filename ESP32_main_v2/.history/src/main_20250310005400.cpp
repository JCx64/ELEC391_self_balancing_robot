 ...
 */
void get_comp_filter_angle(float timeslice){
    ...
    if (IMU.accelerationAvailable() && IMU.gyroscopeAvailable()){ //check accelerometer and gyroscope ready
        ...
        tilt_angle_gyro = theta_zero + gyro_x * timeslice;

        //applying complementary filter to tilt_angle_acc and tilt_angle_gyro to get precise tilt angle
        angle.comp_filter_angle = k*(prev_tilt_angle+gyro_x * timeslice)+(1-k)*tilt_angle_acc; //formula provided may be wrong, modified by J.C.
...
void loop()
{
  ...
  if(currentMillis - previousMillis >= 5){
    ...
    float output_balance = 10.f * (robot_angle.get_pitch()-BALANCE_ANGLE) + 5.f * robot_angle.get_w_yaw();
