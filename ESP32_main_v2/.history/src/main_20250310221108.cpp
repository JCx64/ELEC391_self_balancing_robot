...
void loop()
{
  ...
  if(current_time - prev_time > 5){
    ...
    float output_balance = 20.f * (pitch-BALANCE_ANGLE) + 16.f * imu.getPitchAngularVelocity(); //18

    float velocity_Forward = 3.1415f * 0.08f * (rpm_L + rpm_R) * 0.005f / 30.f;
    float output_velocity = 6.f * (velocity_Forward - 0.f) + 10.f * imu.getXAcceleration();
