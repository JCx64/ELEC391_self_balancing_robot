...
PIDClass robot_velocity_PID(3, 1.2, 0, 0, 50, 100, 0.2, 5.f/1000.f); //0.8
PIDClass robot_rotate_PID(3, 1.2, 0, 0, 50, 100, 0.2, 5.f/1000.f);
...
void loop()
{
  ...
  if(current_time - prev_time > 5){
    ...
    float output_balance = 23.f * (pitch-BALANCE_ANGLE) + 20.f * imu.getPitchAngularVelocity(); //23 20

    float velocity_Forward = 3.1415f * 0.08f * (rpm_L + rpm_R) * 0.005f / 30.f;

    //float output_velocity = 3.f * (velocity_Forward - 0.f) + 1.f * imu.getXAcceleration();
    robot_velocity_PID.pid_setTarget(0);
    ...
    data[0] = pitch;
    data[1] = rpm_R;
    data[2] = imu.getYawAngularVelocity();
