...
void myi2c::I2C_WriteBit(bool bit) {
...
bool myi2c::I2C_WriteByte(uint8_t data) {
    for (int i = 0; i < 8; i++) {
...
uint8_t myi2c::I2C_ReadByte(bool ack) {
    uint8_t data = 0;
    pinMode(SDA_PIN, INPUT_PULLUP);
    for (int i = 0; i < 8; i++) {
...
uint16_t myi2c::readAS5600Angle() {
    I2C_Start();
