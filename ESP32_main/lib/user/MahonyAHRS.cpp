...
char anglesComputed;
//*-*-*-**-----------------------------------------------------------------------------
float invSqrt(float x)  // if use other platform please use float Mahony_invSqrt(float x)
{
...
MahonyClass::MahonyClass(){

}

#define twoKpDef	(2.0f * 0.5f)	// 2 * proportional gain
#define twoKiDef	(2.0f * 0.0f)	// 2 * integral gain
void MahonyClass:: Mahony_Init(float sampleFrequency)
{
...
void MahonyClass::MahonyAHRSinit(float ax, float ay, float az, float mx, float my, float mz)
{
    float recipNorm;
...
void MahonyClass::  Mahony_update(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz)
{
	float recipNorm;
...
void MahonyClass::  MahonyAHRSupdateIMU(float gx, float gy, float gz, float ax, float ay, float az) {
    float recipNorm;
...
void MahonyClass::  Mahony_computeAngles()
{
...
float MahonyClass::  getRoll() {
	if (!anglesComputed) Mahony_computeAngles();
	return roll_mahony;
}
float MahonyClass:: getPitch() {
	if (!anglesComputed) Mahony_computeAngles();
	return pitch_mahony;
}
float MahonyClass:: getYaw() {
	if (!anglesComputed) Mahony_computeAngles();
	return yaw_mahony;
}
