...
{
    float d_Encoder = float(cur_Encoder_Left - pre_Encoder_Left);
...
uint16_t readMagEncoderLeft() {
  Wire.requestFrom(MAG_ENCODER_ADDR, 2);

  if (Wire.available() == 2){
    uint16_t highByte = Wire.read(); // 读取高字节
    uint16_t lowByte = Wire.read();  // 读取低字节
...
{
    cur_Encoder_Left = readMagEncoderLeft();
