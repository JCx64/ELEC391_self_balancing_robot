...
float k = 0.5; //gyroscope weight, can be modified accordingly, based on your condition
//general parameters
...
void loop() {
  if (IMU.accelerationAvailable() && IMU.gyroscopeAvailable()){ //check accelerometer and gyroscope ready
    ...
    tilt_angle_acc = atan2(acc_x,acc_y)*180/pi;
    ...
    tilt_angle = k*(prev_tilt_angle+gyro_z*sample_time)+(1-k)*tilt_angle_acc; //formula may be wrong, changed by myself
