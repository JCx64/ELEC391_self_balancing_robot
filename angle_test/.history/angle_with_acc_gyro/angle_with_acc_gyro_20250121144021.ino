...
float gyro_x, gyro_y, gyro_z, tilt_angle_gyro_now, tilt_angle_gyro_prev=0, theta_zero=0;
//sample time parameters
float sample_time;
unsigned long now_time, prev_time=0;
//complementary filter parameters
float tilt_angle, prev_tilt_angle=0;
float k = 0.99; //gyroscope weight, change accordingly
//general parameters
...
void loop() {
  if (IMU.accelerationAvailable() && IMU.gyroscopeAvailable()){ //check accelerometer and gyroscope ready
    ...
    tilt_angle_gyr_now = theta_zero + gyro_z*sample_time;

    //applying complementary filter to tilt_angle_acc and tilt_angle_gyro to get precise tilt angle
    tilt_angle = k*(prev_tilt_angle+tilt_angle_gyro-tilt_angle_gyro_prev)+(1-k)*tilt_angle_acc;
