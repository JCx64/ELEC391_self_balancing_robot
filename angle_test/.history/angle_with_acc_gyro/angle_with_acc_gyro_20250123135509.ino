...
float k = 0.45; //gyroscope weight, can be modified accordingly, based on your condition
//general parameters
...
void loop() {
  if (IMU.accelerationAvailable() && IMU.gyroscopeAvailable()){ //check accelerometer and gyroscope ready
    ...
    tilt_angle_gyro = theta_zero + gyro_x*sample_time;

    //applying complementary filter to tilt_angle_acc and tilt_angle_gyro to get precise tilt angle
    tilt_angle = k*(prev_tilt_angle+gyro_x*sample_time)+(1-k)*tilt_angle_acc; //formula provided may be wrong, modified by J.C.
